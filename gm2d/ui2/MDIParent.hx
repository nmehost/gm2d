...
class MDIParent extends Widget implements IDock implements IDockable
{
      ...
      else
      {
         ...
         if (idx>=0 && clientArea.getChildIndex(mChildren[idx])<mChildren.length-1)
         {
            ...
            for(child in mChildren)
               child.active = (child.pane==current) ? Active.CURRENT : Active.DORMANT;
         }
      }
      return true;
   }
   ...
   {
      // TODO
      var chrome = new Rectangle(0,0,8,8);
   ...
   {
      widgetLayout.setRect(0,0,inW,inH);
      sizeX = inW;
      sizeY = inH;
      clientArea.x = layW-itemW;
      clientArea.y = layH-itemH;
      /*
...
class MDIChildFrame extends Widget
{
   ...
   public function new(inPane:IDockable, inMDI:MDIParent, inIsCurrent:Bool )
   {
      ...
      var p = pane.asPane();
      var title = p==null ? "" : p.title;
      var ctx = p==null ? "" : "pane:" + p.shortTitle;

      super({ x:x, y:y, title:title, itemWidth:size.x, itemHeight:size.y,
                item:ITEM_LAYOUT(new DockableLayout(inPane,Dock.DOCK_SLOT_FLOAT)) }, ctx);
   ...
   override public function getClass() { return "MDIChild"; }
   ...
   {
      widgetLayout.setRect(0,0, inW+layW-itemW, inH+layH-itemH);
      /*
