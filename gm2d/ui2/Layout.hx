...
class Layout
{
   ...
   public function setGap(inGap:Float) : Layout { return setSpacing(inGap,inGap); }
...
class DockableLayout extends Layout
{
   ...
   public function new(inDockable:IDockable, inDockSlot:Int )
   {
      super();
      dockable = inDockable;
      var pane = inDockable.asPane();
      name = pane==null ? "DockableLayout" : pane.shortTitle;
   ...
   public override function calcSize(inWidth:Null<Float>,inHeight:Null<Float>) : Void
   {
      ...
      {
         width = inWidth;
         height = inHeight==null ? getBestHeight(width) : inHeight;
      }
      else if (inHeight!=null)
      {
         height = inHeight;
         width = inWidth==null ? getBestWidth(height) : inWidth;
      }
      else
      {
         var s = dockable.getBestSize(slot);
         width = s.x;
         height = s.y;
      }
   }
   ...
   {
      dockable.setRect(inX,inY,inW,inH);
   ...
   {
      if (inHeight==null)
         return dockable.getBestSize(slot).x;
      return dockable.getLayoutSize(1,inHeight,false).x;
   }
   public override function getBestHeight(?inWidth:Null<Float>) : Float
   {
      if (inWidth==null)
         return dockable.getBestSize(slot).y;
      return dockable.getLayoutSize(inWidth,1,true).y;
   }
}
...
class DisplayParentLayout extends StackLayout
{
   var object:DisplayObject;

   public function new(inObject:DisplayObject)
   {
      super();
      object = inObject;
   }
