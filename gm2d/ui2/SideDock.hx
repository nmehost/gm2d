...
class SideDock implements IDock implements IDockable
{
   ...
   var mWidgets:Array<Widget>;
   var mSpacers:Array<Widget>;
   ...
   function addPadding(size:Size) : Size
   {
      ...
      {
         for(s in mSpacers)
            size.x += s.getPadWidth();
      }
      else
      {
         for(s in mSpacers)
            size.y += s.getPadHeight();
      }
      return size;
   }
   /*
   ...
   {
      var pane = inDock.asPane();
      if (pane!=null)
      {
         var skin = Dock.isToolbar(pane) ? toolbarSkin : paneSkin;

         var rect = paneSkin.getChromeRect(inDock,toolbarGripperTop);
         ioPos.x += rect.width;
         ioPos.y += rect.height;
      }
      return ioPos;
   ...
   */
   ...
   public function getLayoutSize(w:Float,h:Float,limitX:Bool):Size
   {
      if (isLocked())
      {
         ...
         {
            var dock = mDockables[d];
            var pad = mWidgets[d].addPadding(new Size(0,0));
            var s = dock.getLayoutSize(w-pad.x, h-pad.y,!horizontal);
            s.x+=pad.x;
            s.y+=pad.y;
   ...
   public function setRect(x:Float,y:Float,w:Float,h:Float):Void
   {
      ...
      if (isLocked())
      {
         for(di in 0...mDockables.length)
         {
            var d = mDockables[di];
            var chrome = mWidgets[di].addPadding(new Size(0,0));

            var s = d.getLayoutSize(w-chrome.x,h-chrome.y,!horizontal);
            s.x+=chrome.x;
            s.y+=chrome.y;
            mSizes.push(s);
      ...
      else
      {
         ...
         while(locked_changed)
         {
            ...
            for(d in 0...mDockables.length)
            {
               ...
               var chrome = mWidgets[d].addPadding(new Size(0,0));
               var layout_w = (horizontal?size:w) - chrome.x;
               var layout_h = (horizontal?h:size) - chrome.y;
      ...
      for(d in 0...mDockables.length)
      {
         ...
         var dw = (horizontal?size:w);
         var dh = (horizontal?mSizes[d].y:size);

         var oid = indent;
         indent+="   ";
         mWidgets[d].widgetLayout.setRect(x,y,dw,dh);
         indent = oid;

         if (horizontal)
         {
            mPositions.push( x );
            if (d<mSpacers.length)
            {
               var w = mSpacers[d].getPadWidth();
               mSpacers[d].widgetLayout.setRect(x,y,w,h);
               x+=size + w;
            }
         }
         else
   ...
   */
   ...
   public function getDockableRect(inRef:Int)
   {
      ...
      for(i in 0...inRef)
      {
         origin += mWidths[i];
         if (i>0 && i<inRef-1)
           origin += horizontal ? mSpacers[i].getPadWidth() : mSpacers[i].getPadHeight();
      }
